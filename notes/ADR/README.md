...
- ADR-015 权限管理做一层认证（登录态 + 命令下发校验/审计）—— 已落地｜读
- ADR-016 上行载荷携带点位权限，云端回填可写性并展示 —— 已落地（网关透传 + 解析 + Writable 回填 + 大屏徽标）｜读
- ADR-017 元数据只读约束（站点/设备/点位禁止手动新增/删除，由自动注册驱动）—— 已落地（后端开关+403、前端去新增/删除入口、单测/集成测试）｜读
